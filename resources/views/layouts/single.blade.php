...
<head>
    ...
    <script>
        const allowedDates = @json($allowedDates);
    </script>
</head>
<body>
...
<script>
    ...
    // Поиск ближайшей разрешённой даты после checkin
    function getNearestCheckoutDate(checkinDate) {
        for (let d of allowedDates) {
            if (d >= checkinDate) return d;
        }
        return null;
    }
    ...
    checkin.addEventListener('change', () => {
        if (!validateAllowed(checkin)) return;

        checkout.min = checkin.value;

        const autoCheckout = getNearestCheckoutDate(checkin.value);
        if (autoCheckout) {
...
<script>
    const priceSpan = document.getElementById('yourPrice');
    const dailyPrice = parseFloat(priceSpan.innerText); // цена за сутки
    ...
    function updateTotalPrice() {
        ...
        if (days > 0) {
            priceSpan.innerText = dailyPrice * days;
        } else {
            priceSpan.innerText = dailyPrice; // если дат нет — показываем базовую цену
        }
    }

    // Вызов обновления при изменении дат
...
<script>
    ...
    // При загрузке страницы — блокируем все чекбоксы
    serviceCheckboxes.forEach(ch => ch.disabled = true);

    // Разблокируем, когда выбран checkout
    checkout.addEventListener('change', () => {
        if (checkout.value) {
            serviceCheckboxes.forEach(ch => ch.disabled = false);
        } else {
